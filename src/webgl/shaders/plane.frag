...
// zoomIn 発火時に併走するブースト係数 (0..1)。RGB シフト + スライスグリッチを増強
	...
	if (!gl_FrontFacing) {
		...
		if (vUv.x < borderUV.x || vUv.x > 1.0 - borderUV.x ||
			...
			if (uGlitchRadius > 0.0) {
				...
				// zoomIn 時 (uZoomBurst > 0) は RGB シフト / スライスを強めに
				float burstBoost = 1.0 + uZoomBurst * 0.9;
				...
				float amp = ABERRATION_STRENGTH * burstBoost * ringMask * envelope * glitchAmount;
				// 主に水平方向、少しだけ縦揺れを混ぜる
				...
				float sliceShift = sign(sliceNoise) * sliceAmount
					* SLICE_OFFSET_STRENGTH * burstBoost * ringMask * envelope;
